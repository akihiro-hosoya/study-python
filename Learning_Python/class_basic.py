...
# __init__メソッド
"""
あるクラス（やそのインスタンス）に固有の処理を実行するために使われる関数のこと
...
#   インスタンスの初期化処理を行うコード
#   self.param = param
# 第2パラメーター以降にはインスタンスの初期化に必要となるものを列挙していく
# クラスのインスタンス生成時に「クラス名()」形式の関数を呼び出してその引数に指定したものが、
# __init__メソッドに渡されるようになっている
class Point:
    def __init__(self, x=0.0, y=0.0):
        self.x = x
        self.y = y
        # インスタンスの属性xにパラメーターxの値が、
        # 属性yにパラメーターyの値が代入される

# 上のコードではデフォルト引数値を指定しているので、
# 「Point()」としてインスタンスを生成したときには、
# 2つのパラメーターには「0.0」が渡される
...
# point1が示す座標「(1.0, 1,0)」とpoint2が示す
# 座標「(0.0, 0.0)」との距離も求めてみる
